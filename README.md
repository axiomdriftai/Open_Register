...
Boundedness (whether variability remains constrained)
Persistence (whether stability is sustained across time)
Time-to-stability (the point at which a stable regime emerges, if at all)
...
Accept user-supplied conversational data in common export formats
Observe interaction-level behavior across scale
Detect regime transitions (exploratory → convergent → unstable)
Test the robustness of detected structure under controlled perturbation
